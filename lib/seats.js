...
function normalizeSeatEntry(raw) {
  ...
  if (typeof raw === 'string') {
    if (raw === 'selected') return { status: 'assigned', name: '', paid: true, concessionsPaid: false, concessions: [] };
  ...
  if (typeof raw === 'object') {
    if (raw.status === 'assigned') {
      ...
      return {
        ...
        // Whether the food's been settled, tracked apart from the ticket
        // because the two get paid at different times: the ticket's
        // amount is known the moment the seat is claimed, the food's only
        // once the order goes in. See the cutoff in views/public.html.
        concessionsPaid: name ? !!raw.concessionsPaid : false,
...
// someone sends over once the order's gone in.
